...
#   with byte[0] == 0x01, the frame-version sentinel). Exits 0 on PASS, non-zero
#   on FAIL.
#
...
#   FRAME_LEN        — expected payload length in bytes       (17)
#
# Requires on gaia:
...
while time.time() < end:
    ...
    if not poller.poll(remaining_ms):
        continue
    data = os.read(fd, mtu)
    if len(data) == frame_len and data[0] == 0x01:
        good += 1
    elif data:
        bad += 1
...
case "$rc" in
    ...
    2) fail "malformed frame(s) received — wrong length or byte[0] != 0x01" ;;
