...
impl<'stack, P: PacketPool> Connection<'stack, P> {
        ...
        T: ControllerCmdAsync<LeConnUpdate> + ControllerCmdSync<LeReadLocalSupportedFeatures>,
    {
        ...
        // First, check the local supported features to ensure that the connection update is supported.
        let features = stack.host.command(LeReadLocalSupportedFeatures::new()).await?;
        // ESP32-H2 workaround (meteo_station vendored patch): the on-chip controller
        // advertises support for the LL connection-parameters-request procedure, so the
        // upstream condition (`supports(...) || role == Central`) makes a *peripheral*
        // issue an HCI LE Connection Update. The H2 controller returns Command Status =
        // success but never runs the procedure on air — no LE Connection Update Complete
        // ever arrives, the parameters never change, and the link stays at the central's
        // short default supervision timeout (~420 ms on BlueZ), dropping on any brief
        // central-radio stall. Restrict the HCI path to the central role; a peripheral
        // falls through to the L2CAP Connection Parameter Update signaling below, which
        // the controller forwards correctly.
        if self.role() == LeConnRole::Central && features.supports_conn_parameters_request_procedure() {
